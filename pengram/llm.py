...
def _call_claude_cli(
    ...
    *,
    ...
    try:
        ...
        raise LLMError(
            "claude CLI not found. Install Claude Code or switch "
            "LLM_PROVIDER to 'openai' or 'openrouter'."
        ) from exc
...
def _call_ollama(
    ...
    *,
    ...
    try:
        ...
        raise LLMError(f"Ollama request timed out after {timeout}s") from exc
...
def call_llm(
    ...
    *,
    ...
    if provider == "ollama":
        # ``_auto`` (the config default) triggers detection via
        # ``/api/ps`` → ``/api/tags``. An explicit --llm-model overrides.
        if not model or model == "_auto":
...
__all__ = ["LLMError", "call_llm", "parse_json_response"]
